...
serve(async (req) => {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            const verifyToken = Deno.env.get('FB_VERIFY_TOKEN') || Deno.env.get('WHATSAPP_VERIFY_TOKEN')
        ...
        if (req.method === 'POST') {
            const payload: MessengerWebhook = await req.json()

            console.log('Received Messenger webhook:', JSON.stringify(payload, null, 2))
                ...
                for (const messagingEvent of entry.messaging) {
                    try {
                        ...
                        const { data: lead, error: leadError } = await supabase
                            ...
                            .upsert({
                                fb_psid: senderId,
                                full_name: `FB User ${senderId.substring(0, 8)}`,
                                metadata: { last_contact: new Date().toISOString(), platform: 'messenger' }
                            }, {
                                onConflict: 'fb_psid',
                        ...
                        console.log(`Messenger message processed successfully for conversation ${conversation.id}`)
                    } catch (error) {
        ...
        console.error('Error in messenger-inbound:', error)
